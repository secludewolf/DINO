...
def check_file(file, suffix=''):
    ...
    if os.path.isfile(file) or not file:  # exists
        ...
        files = []
        for d in 'data', 'models_yolo', 'utils':  # search directories
...
def check_amp(model):
    ...
    from models_yolo.common import AutoShape, DetectMultiBackend
...
def non_max_suppression(
    ...
    bs = prediction.shape[0]  # batch size  批量大小
    nc = prediction.shape[2] - nm - 5  # number of classes  类别个数
    xc = prediction[..., 4] > conf_thres  # candidates  置信度

    # Settings
    # min_wh = 2  # (pixels) minimum box width and height  最小高宽
    max_wh = 7680  # (pixels) maximum box width and height  最大高宽
    max_nms = 30000  # maximum number of boxes into torchvision.ops.nms()  单个图片最多目标个数
    time_limit = 0.5 + 0.05 * bs  # seconds to quit after  最长NMS执行时间,超过就退出
    redundant = True  # require redundant detections  是否需要让冗余的
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if not n:  # 如果经过第三轮过滤该feature map没有目标框了，就结束这轮直接进行下一张图  # no boxes
            continue
...
def apply_classifier(x, model, img, im0):
    ...
    # Example model = torchvision.models_yolo.__dict__['efficientnet_b0'](pretrained=True).to(device).eval()
